...
exports.addProject = async (req, res) => {
  ...
  // access userId from requested payload
  ...
  try {
    ...
    if (existingProject) {
      res
        .status(400)
        .json(`${existingProject.title} is already exist! add new one`);
    } else {
...
exports.getUserPojects = async (req, res) => {
  const userId = req.payload;
...
exports.editProject=async(req,res)=>{
 ...
 try{ const updatedProject = await projects.findByIdAndUpdate({_id},
...
exports.deleteProject=async(req,res)=>{
  ...
  try{const deletedProject = await projects.findByIdAndDelete({_id})
